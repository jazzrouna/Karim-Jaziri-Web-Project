...
const BASE_URL = "http://localhost:6989/"
...
export const login = async ({ email, password, handleLoginSuccess, handleLoginFailure }) => {
    try {
        const result = await axios.post(`http://localhost:6989/user/login`, { email, password }) // login port issue solved 
...
export const register = async ({ name, email, password, type, handleRegisterSuccess, handleRegisterFailure }) => {
    try {
        ...
        if (result?.data?.name) {
            ...
            // Handle a specific scenario of registration failure
            handleRegisterFailure('Registration failed');
        }
    } catch (error) {
        console.error('register ', error);
        // Extract the error message and pass it to the failure handler
...
export const fetchSpaces = async ({ user_id, parking_id, city, date, time, availability, setSpaces }) => {
    try {
        ...
        const result = await axios.get(`${BASE_URL}sapi?${query}`)
        if (result?.data?.length) {
...
export const createSpace = async ({ body, handleCreateSpaceSuccess, handleCreateSpaceFailure }) => {
    try {
        const result = await axios.post(`${BASE_URL}sapi`, { ...body })
...
export const updateSpace = async ({ id, body, handleUpdateSpaceSuccess, handleUpdateSpaceFailure }) => {
    try {
        const result = await axios.put(`${BASE_URL}sapi/${id}`, { ...body })
        if (result?.data?.message) {
...
export const fetchBookings = async ({ admin_id, user_id, setBookings }) => {
    try {
        let query = '';
        if(user_id){
            query += `user_id=${user_id}&`;
...
export const deleteSpace = async ({ id, handleDeleteSpaceSuccess, handleDeleteSpaceFailure }) => {
    try {
        const result = await axios.delete(`${BASE_URL}sapi/${id}`)
        if (result?.data?.message) {
...
export const fetchReviews = async ({ user_id, setReviews }) => {
    try {
        let query = '';
        if (user_id) {
            query += `?user_id=${user_id}`
        }
        const result = await axios.get(`${BASE_URL}api${query}`)
        if (result?.data?.length) {
